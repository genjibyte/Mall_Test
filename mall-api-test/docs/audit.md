...
### 🔴 H1 · 测试数据隔离不足 / 状态漂移（最高优先）
- **现象（实测）**：4 个 SKU `lock_stock<0`；member test 累计 171 笔订单(19 完成/111 关闭)、226 条软删购物车行。
- **根因**：多用例共享 member test + 同一无促销 SKU(163)，靠 before/after 差值断言 + 手动 setStock/setLockStock 还原；无 DB 事务回滚包裹，用例中途失败会泄漏状态；R1/R4 探针造成负库存。
- **影响**：长期运行数据膨胀；个别用例对绝对值敏感时可能受污染；不利于并行。
- **建议**：① 每用例用 DB 事务包裹并回滚（或 Testcontainers 全新库）；② 或为高风险用例分配独立会员/商品；③ 增加 suite 级 teardown 统一清理（关闭订单、复位负库存、清软删购物车）。
...
**总体**：架构与断言质量高，是一套可直接用于回归/CI 的专业框架。最大技术债集中在**测试数据隔离/漂移(H1/H2)**——建议在继续扩规模或开并行前优先治理。

改进优先级：**H1 数据隔离 → H2 专用测试账号 → M3 MQ 异步用例 → M5 admin 覆盖 → M1/M2 → L\***。
