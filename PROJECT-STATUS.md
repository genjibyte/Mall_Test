...
GitHub：https://github.com/genjibyte/Mall_Test （`main`，21 提交）。
...
| `mall-api-test/` | 测试框架（Java+JUnit5+RestAssured+Allure），44 用例（默认 43 + 1 @slow MQ）|
...
4. **测试框架**：分层 client/flow/fixture/core/cases；**43 用例（37 通过 + 6 跳过：5 缺陷探针 + 1 数据维护）** 覆盖 5 核心链路 + 浏览/会员/购物车 + 跨服务 + 后台管理(商品批量状态/订单关闭) + 测试基建(库存完整性守卫)。
...
cd mall-api-test && mvn test                                   # 默认 43 用例全绿(6 跳过；slow 排除)
...
2. admin 管理端链路续：审核(verifyStatus)、退货审核、批量删除/恢复（M5 剩余；批量上下架/新品/推荐/关单已覆盖）。
