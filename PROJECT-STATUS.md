...
GitHub：https://github.com/genjibyte/Mall_Test （`main`，22 提交）。
...
| `mall-api-test/` | 测试框架（Java+JUnit5+RestAssured+Allure），46 用例（默认 45 + 1 @slow MQ）|
...
4. **测试框架**：分层 client/flow/fixture/core/cases；**46 用例（默认 45：39 通过 + 6 跳过；另 1 @slow MQ）** 覆盖 5 核心链路 + 浏览/会员/购物车 + 跨服务 + 后台管理(商品批量状态/审核/软删/订单关闭) + 测试基建(库存完整性守卫) + MQ 真实延迟。
5. **缺陷发现**：R1 非幂等支付、R2 越权支付、R4 并发超卖、**R6 积分不退**、**R8 管理员关单不退锁库存(库存泄漏)**——均为 @KnownDefect 探针；另 R3 非事务、R7 空车clear500 已记录。
6. **H1 数据漂移治理**：DataHygieneFixture + DataIntegrityTest(常驻守卫) + DataMaintenanceTest(手动)；已清理负库存 4→0、软删购物车 243→0。
...
cd mall-api-test && mvn test                                   # 默认 45 用例全绿(6 跳过；slow 排除)
...
1. **H1 续**：专用测试账号/商品(替代共享 test) + Testcontainers 全新库 + 开并行（损伤清理/守卫已就绪）。
2. admin 管理端链路续：退货审核(oms_order_return)、SKU 库存维护、改单（M5 剩余；上下架/新品/推荐/审核/软删/关单已覆盖）。
3. 会员注册(短信码)、搜索综合筛选(brandId/category/sort)、R3 非事务探针。
