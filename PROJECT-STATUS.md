...
GitHub：https://github.com/genjibyte/Mall_Test （`main`，26 提交）。
...
6. **H1 数据漂移治理**：DataHygieneFixture + DataIntegrityTest(常驻守卫) + DataMaintenanceTest(手动)；已清理负库存 4→0、软删购物车 243→0。**专用隔离 SKU**(IsolatedSkuFixture)落地库存层隔离，会员级隔离待续。
...
1. **H1 续**：会员级隔离(新建专用 member 承接订单累积) + 既有库存类用例迁移到 IsolatedSkuFixture + Testcontainers/并行（专用 SKU 隔离/守卫/维护已就绪）。
