...
> ⚠️ **ES on WSL2 cgroup v2（2026-06-19）**：`wsl --update` 把 cgroup 切到 v2 后，**ES 7.17.3 自带 JDK 在 `CgroupV2Subsystem.getInstance` NPE 崩溃**（`anyController is null`，启动器读容器内存即崩）→ 容器 crash-loop。**仅影响搜索链路**(SearchChain/SearchFilterSort/AdminProductSearch 共 9 用例)；下单/其余 **50 用例 live 全绿不受影响**。搜索用例代码经史验证、无需改动。
> **本会话实测无效的修法**（均试过、均不解，勿重复）：① `.wslconfig` + `kernelCommandLine=systemd.unified_cgroup_hierarchy=0`（Docker Desktop 的 `docker-desktop` 分发非 systemd，忽略该参数）；② compose `cgroup: host`；③ `JAVA_TOOL_OPTIONS=-XX:-UseContainerSupport`（ES entrypoint 不向启动器透传，无 "Picked up"）。
> **真正修法（未施加，留待需要搜索时做）**：把 ES 镜像 **升到 7.17.x 较新补丁**（如 ≥7.17.10，自带 JDK ≥17.0.4 已修复该 cgroup v2 bug）并装**对应版本** IK（`https://get.infini.cloud/elasticsearch/analysis-ik/<版本>`）→ `importAll`。需拉新镜像，故未在本次闭环内做。
>
...
1. **H1 续**：既有库存类用例渐进迁移到 Isolated* 夹具 + Testcontainers 全新库 + 开并行（双隔离夹具/守卫/维护/回收已就绪）。
2. admin 管理端链路续：SKU 库存维护、订单收货人/费用改单（M5 剩余；上下架/新品/推荐/审核/软删/关单/退货审核已覆盖）。
3. 会员注册(短信码)、搜索推荐/relate、R3 非事务探针。
4. 落地 CI（context-pack/08 设计已就绪；slow 用例走 nightly `-Pslow`）。
