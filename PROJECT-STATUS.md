...
GitHub：https://github.com/genjibyte/Mall_Test （`main`，17 提交）。
...
| `mall-api-test/` | 测试框架（Java+JUnit5+RestAssured+Allure），37 用例 |
...
4. **测试框架**：分层 client/flow/fixture/core/cases；**37 用例（33 通过 + 4 @KnownDefect 跳过）** 覆盖 5 核心链路 + 浏览/会员/购物车 + 跨服务。
5. **缺陷发现**：R1 非幂等支付、R2 越权支付、R4 并发超卖、**R6 积分不退(测试新发现)**——均为 @KnownDefect 探针；另 R3 非事务、R7 空车clear500 已记录。
...
cd mall-api-test && mvn test                                   # 37 用例全绿(4 跳过)
```
...
- 其余：MQ 真实延迟路径未测(M3)、admin 覆盖薄(M5)、硬编码锚点(M1)、并行禁用(M2)。

## 7. 下一步建议（按价值）

1. **治理测试数据隔离**（H1/H2）——专用测试账号/商品 + 事务回滚 + suite teardown 清理。
2. MQ 超时取消 Awaitility @Tag("slow") 用例（M3）。
3. admin 管理端链路：批量上下架→状态流转、关单/退货审核（M5）。
4. 会员注册(短信码)、搜索综合筛选、R3 非事务探针。
5. 落地 CI（context-pack/08 设计已就绪）。

## 8. Agent 长期记忆指针

`~/.claude/projects/E--Mall-Test/memory/`：mall-swarm-test-framework / mall-swarm-local-deployment / order-chain-findings（含 R1–R7 与数据锚点）。
