...
> ⚠️ **ES on WSL2 cgroup v2（2026-06-19 新增）**：`wsl --update`(WSL≥2.x 内核) 把 cgroup 切到 v2 后，**ES 7.17.3 自带 JDK 在 `CgroupV2Subsystem.getInstance` NPE 崩溃**（`anyController is null`），容器 crash-loop，`JAVA_TOOL_OPTIONS=-XX:-UseContainerSupport` 不生效。**仅影响搜索链路**(SearchChain/SearchFilterSort/AdminProductSearch ~9 用例)，下单/其余 50 用例不受影响。
> **可靠修法**：建 `%USERPROFILE%\.wslconfig` 写 `[wsl2]` + `kernelCommandLine = systemd.unified_cgroup_hierarchy=0`（强制 cgroup v1）→ `wsl --shutdown` → 重启 Docker → ES 正常 → 重装 IK → `importAll`。本次为避免再次打乱刚恢复的 Docker，未现场施加。
>
